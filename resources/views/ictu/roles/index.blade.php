                            ...
                            <tbody>
                                @if(sizeof($user_roles) > 0)
                                    @foreach($user_roles as $user_role)
                                        <tr>
                                            <td><img src="{{ asset('storage/avatars') }}/{{ $user_role->user->person->image }}" width="40" class="img-circle"></td>
                                            <td>
                                                <a href="{{ route('ictu.roles.show', $user_role->id ) }}">
                                                    {{ $user_role->user->name }}
                                                </a>
                                            </td>
                                            <td>{{ $user_role->user->username }}</td>
                                            <td>{{ $user_role->getRole($user_role->role_id) }}</td>
                                            <td>{{ $user_role->route }} </td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
